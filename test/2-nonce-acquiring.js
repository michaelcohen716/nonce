...
contract('Nonce', async (accounts) => {
    ...
    it('Should let a user acquire a free token', async () => {
        await nonce.mintToken(web3.utils.fromAscii("FreeToken"), 100, 0, { from: alice });
    ...
    it('Should let a user acquire a non-free token', async () => {
        ...
        await nonce.mintToken(web3.utils.fromAscii("ExpensiveToken"), 100, price, { from: alice });
    ...
    it('Should decrement available supply when a fixed supply token is acquired', async () => {
        const supply = 100;
        await nonce.mintToken(web3.utils.fromAscii("FixedSupplyToken"), supply, 0, { from: alice });
    ...
    it('Should not let a user acquire a fixed supply token with less than available quantity', async () => {
        const supply = 2;
        await nonce.mintToken(web3.utils.fromAscii("LimitedFixedSupplyToken"), supply, 0, { from: alice });
